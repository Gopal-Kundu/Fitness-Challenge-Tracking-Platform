  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        dispatch(setUser({ name, email, role, tier: 'ELITE' }));
      }
      if (role === 'admin') {
        navigate('/admin');
    ...
    <div className="min-h-screen bg-background text-on-background flex flex-col items-center justify-center p-6 relative">
      ...
      <div className="bg-surface-container border border-outline-variant rounded-2xl max-w-md w-full p-8 space-y-6 shadow-2xl">
        <div className="text-center space-y-2">
          <h1 className="font-display-lg text-3xl text-primary font-bold">JOIN APEX ATHLETICS</h1>
          <p className="font-body-md text-sm text-on-surface-variant">
            Create your account and select your platform role.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-surface-container-low border border-outline-variant text-on-surface px-4 py-3 rounded-lg focus:border-primary-container outline-none font-body-md cursor-pointer"
            >
