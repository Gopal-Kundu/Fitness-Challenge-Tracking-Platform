...
function ChallengeModal({ challenge, onClose }: any) {
  ...
  const isAlreadyJoined =
    ...
    (Array.isArray(currentUser?.joinedChallenges) &&
      currentUser.joinedChallenges.some(
        (id: any) => String(id) === String(challengeId) || String(id) === String(challenge?.title)
      ));
  ...
  const handleJoin = async () => {
    ...
    try {
      const res: any = await challengeAPI.joinChallenge(challengeId);
